...
// 気配察知はユニット全員が行うが、誰か一人が成功すればよい（bool値で誰かが成功したらtrueを返せばよい)
// 索敵演出(立ち絵表示)は、部隊で一番索敵に向いてるユニットが表示される(気配察知に失敗していてもよい)
...
public class Search_Controller : MonoBehaviour
{
    ...
    public IEnumerator PhaseCtrlCoroutine ()
    {
    ...
    public void SetSearchPhaseResult ( bool isWinPlayer )
    {
        gameController.GetSearchPhaseResult ( isWinPlayer );
        // GameControllerのBattlePhaseControllCoroutineのyield return自体に
        // 戻り値を設定できる方法が判れば、この関数を削除してください。
